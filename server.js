...
// Используем системный yt-dlp (устанавливается через build command: pip install yt-dlp)
...
// ВАЖНО: trust proxy для Render.com (они используют прокси)
...
app.use('/api/', rateLimit({
  ...
  legacyHeaders: false,
  trustProxy: true,  // ВАЖНО: для корректной работы за прокси
...
/**
 * Получить информацию о видео через spawn yt-dlp
 * (вместо yt-dlp-exec который ищет бинарник в node_modules)
 */
function getVideoInfo(url) {
  return new Promise((resolve, reject) => {
    ...
    ytProcess.on('error', (err) => {
      reject(new Error(`Failed to start yt-dlp: ${err.message}. Убедитесь что yt-dlp установлен (pip install yt-dlp)`));
    });
  });
}
